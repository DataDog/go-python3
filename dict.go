...
func PyDict_Contains(p, key *PyObject) (bool, error) {
	if ret := C.PyDict_Contains(toc(p), toc(key)); ret != -1 {
		return ret != 0, nil
	}
	return false, fmt.Errorf("PyDict_Contains: an error occured")
}
...
func PyDict_SetItem(p, key, val *PyObject) error {
	if C.PyDict_SetItem(toc(p), toc(key), toc(val)) == -1 {
		return fmt.Errorf("PyDict_SetItem: an error occured")
	}
	return nil
}

//PyDict_SetItemString : https://docs.python.org/3/c-api/dict.html#c.PyDict_SetItemString
func PyDict_SetItemString(p *PyObject, key string, val *PyObject) error {
	ckey := C.CString(key)
	defer C.free(unsafe.Pointer(ckey))
	if C.PyDict_SetItemString(toc(p), ckey, toc(val)) == -1 {
		return fmt.Errorf("PyDict_SetItemString: an error occured")
	}
	return nil
}

//PyDict_DelItem : https://docs.python.org/3/c-api/dict.html#c.PyDict_DelItem
func PyDict_DelItem(p, key *PyObject) error {
	if C.PyDict_DelItem(toc(p), toc(key)) == -1 {
		return fmt.Errorf("PyDict_DelItem: an error occured")
	}
	return nil
}

//PyDict_DelItemString : https://docs.python.org/3/c-api/dict.html#c.PyDict_DelItemString
func PyDict_DelItemString(p *PyObject, key string) error {
	ckey := C.CString(key)
	defer C.free(unsafe.Pointer(ckey))
	if C.PyDict_DelItemString(toc(p), ckey) == -1 {
		return fmt.Errorf("PyDict_DelItemString: an error occured")
	}
	return nil
}
